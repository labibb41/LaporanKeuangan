<x-guest-layout>
    <div class="mx-auto w-full max-w-md">
        ...
        <div class="mb-6 flex rounded-2xl bg-stone-100 p-1">
            <a href="{{ route('login') }}" class="flex-1 rounded-xl px-4 py-2 text-center text-sm font-semibold text-white shadow-sm transition" style="background-color: #164A41;">Login</a>
            @if (Route::has('register'))
                <a href="{{ route('register') }}" class="flex-1 rounded-xl px-4 py-2 text-center text-sm font-semibold text-stone-700 hover:text-stone-950 transition">Daftar</a>
            @else
        ...
        <div x-data="{ loading: false }" class="relative">
            ...
            <button type="submit" class="mt-2 flex w-full items-center justify-center gap-3 rounded-2xl px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:opacity-90 disabled:opacity-70 disabled:cursor-not-allowed" style="background: linear-gradient(135deg, #164A41, #4D774E);" x-bind:disabled="loading">
